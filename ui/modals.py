...
from core.db.methods import (add_item, add_user, change_analysis_setting,
                             ...
                             get_last_added_user, get_secrets, get_setting,
                             get_users)
from core.db.models import Item, User
from core.threads import DeleteUnprofitableThread, ProgressBarThread
...
from steamlib.models import APIEndpoint, Tag
...
class AddItemModalWindow(QtWidgets.QMainWindow):
    ...
    def validate_url(self) -> Item:
        try:
            ...
            self.url_error_message.setText(
                "Bad url (Invalid item url or game doesn't exist in the database or this item already in the database)."
            )
            return False
        return item
...
class LoginModalWindow(QtWidgets.QMainWindow):
    ...
    def setupUi(self):
        ...
        self.policy = QtWidgets.QSizePolicy(
    ...
    def login(self):
        ...
        if isinstance(resp, requests.Session):
            if self.account_select:
                SelectAccountCodeModalWindow(
                    client, self, self.account_modal
                ).send_code(resp, need_code=False)
            else:
                CodeModalWindow(client, self, self.main_win).send_code(
                    resp, need_code=False
                )
            return
        ...
        ) and "too many login failures" not in resp.get("message", ""):
            self.password_input.clear()
            self.login_error_message.setText(resp.get("message", ""))
            self.setFixedSize(441, 370)
            self.captcha = QtGui.QImage()
            captcha_gid = resp.get("captcha_gid")
            self.captcha.loadFromData(
                requests.get(
                    f"{APIEndpoint.COMMUNITY_URL}login/rendercaptcha/?gid={captcha_gid}"
                ).content
            )
        ...
        if resp.get("requires_twofactor", False) or resp.get("emailauth_needed", False):
            ...
            if self.account_select:
                self.code_window = SelectAccountCodeModalWindow(
                    client, self, self.account_modal, is_email_need=email
                )
            else:
                self.code_window = CodeModalWindow(
                    client, self, self.main_win, is_email_need=email
                )
...
class CodeModalWindow(QtWidgets.QMainWindow):
    def __init__(
        self, client, login_modal_window, main_window=None, is_email_need=False
    ):
        super().__init__()
        self.client = client
    ...
    def send_code(self, session: requests.Session, need_code: bool = True):
    ...
    def get_response(self, session: requests.Session, need_code: bool):
        if need_code:
            if self.email:
...
class AccountSelectModalWindow(QtWidgets.QMainWindow):
    ...
    def set_geometry(self):
        ...
        self.account_scrollAreaWidgetContents.setGeometry(
            QtCore.QRect(*self.params["scroll_widget"])
        )
    ...
    def title(self):
        ...
        self.account_title.setText("Select account")
    ...
    def scroll(self):
        ...
        sizePolicy.setHeightForWidth(
            self.account_scrollArea.sizePolicy().hasHeightForWidth()
        )
        ...
        self.account_scrollAreaWidgetContents.setObjectName(
            "account-scrollAreaWidgetContents"
        )
        self.frame()
    ...
    def frame(self):
        self.verticalLayout = QtWidgets.QVBoxLayout(
            self.account_scrollAreaWidgetContents
        )
    ...
    def create_frame(self, account: User):
    ...
    def action_buttons(self, frame: QtWidgets.QFrame, account: User):
    ...
    def delete_account(self, frame: QtWidgets.QFrame, account: User):
        delete_user(account)
        if account.is_login:
            self.restart()
    ...
    def switch_account(self, account: User):
        change_user(account)
        self.restart()
...
class ConfirmModalWindow(QtWidgets.QMainWindow):
    ...
    def confirms(self):
        ...
        self.confirm_all_checkbox.clicked.connect(
            lambda: self.select_all(self.confirm_all_checkbox.isChecked())
        )
    ...
    def confirm(self):
        ...
        for item in self.items:
            ...
            sizePolicy.setVerticalStretch(0)
            sizePolicy.setHeightForWidth(
                self.confirm_card.sizePolicy().hasHeightForWidth()
            )
            ...
            self.confirmation_list.append(
                {
                    "checkbox": self.confirm_checkbox,
                    "item": item,
                    "frame": self.confirm_card,
                }
            )
    ...
    def confirm_items(self, tag: str):
        for confirm, index in zip(
            self.confirmation_list, range(len(self.confirmation_list))
        ):
    ...
    def delete_confirm_card(self, frame: QtWidgets.QFrame):
        self.confirm_layout.removeWidget(frame)
        sip.delete(frame)

    def get_picture(self, item_picture: str, card: QtWidgets.QFrame):
...
class SettingsModalWindow(QtWidgets.QMainWindow):
    def __init__(self, guard, main_win, parent=None):
        ...
        self.autoconfirm_checkbox.setChecked(self.setting["autoconfirm"])
        self.autoconfirm_checkbox.clicked.connect(
            lambda: self.switch_autoconfirm(self.autoconfirm_checkbox.isChecked())
        )

        self.analisys_lbl = QtWidgets.QLabel(self)
        self.analisys_lbl.setText("Analisys")
        self.analisys_lbl.setGeometry(15, 95, 100, 20)
        self.analisys_lbl.setObjectName("settingsAnalisysLabel")

        self.analysis_checkbox = AnimatedToggle(
            self,
            checked_color="#3c50fa",
            bar_color=QtGui.QColor(69, 72, 80),
            handle_color=QtGui.QColor(172, 172, 174),
        )
        self.analysis_checkbox.setGeometry(130, 85, 60, 40)
        self.analysis_checkbox.setChecked(self.setting["analysis"])
        self.analysis_checkbox.clicked.connect(
            lambda: self.switch_analysis(self.analysis_checkbox.isChecked())
        )

        self.guard_lbl = QtWidgets.QLabel(self)
        self.guard_lbl.setText("Steam guard")
        self.guard_lbl.setObjectName("settingsGuardLabel")
        self.guard_lbl.setGeometry(15, 135, 100, 20)

        self.guard_btn = GuardButton(self.status["state"], parent=self)
        self.guard_btn.setGeometry(138, 130, 40, 30)
        self.guard_btn.clicked.connect(self.guard_action)
    ...
    def guard_action(self):
        if not self.status["state"]:
            response = self.guard.add()
            data = response["response"]
            self.sms_code_window = SMSCodeModalWinow(data, self.guard, self.main_win)
            self.sms_code_window.setupUi()
            self.sms_code_window.show()
        else:
            revocation_code = get_secrets()["revocation_code"]
            self.guard.remove(revocation_code)
            change_secrets()
            self.main_win.restart()

    def switch_analysis(self, state: bool):
        change_analysis_setting(state)

    def switch_autoconfirm(self, state: bool):
        change_autoconfirm_setting(state)


class SMSCodeModalWinow(QtWidgets.QMainWindow):
    def __init__(self, data, guard, main_win):
        super().__init__()
        self.data = data
        self.guard = guard
        self.main_win = main_win
    ...
    def label(self):
        self.sms_code_label = QtWidgets.QLabel(self)
        self.sms_code_label.setObjectName("sms-title")
        self.sms_code_label.setGeometry(QtCore.QRect(0, 0, 401, 41))
        self.sms_code_label.setAlignment(QtCore.Qt.AlignCenter)
        self.sms_code_label.setText("SMS CODE")

    def code_input(self):
        self.sms_code_input = QtWidgets.QLineEdit(self)
        self.sms_code_input.setObjectName("default-input")
        self.sms_code_input.setGeometry(QtCore.QRect(20, 60, 361, 31))
        self.sms_code_input.setPlaceholderText("Code")

    def button(self):
        self.sms_code_button = QtWidgets.QPushButton(self)
        self.sms_code_button.setObjectName("default-btn")
        self.sms_code_button.setText("OK")
        self.sms_code_button.setGeometry(QtCore.QRect(150, 100, 100, 31))
        self.sms_code_button.clicked.connect(self.activate_code)

    def activate_code(self):
        code = self.sms_code_input.text()
        self.guard.finalize(code)
        change_secrets(self.data)
        self.main_win.restart()
